...
** Experiments
:PROPERTIES:
:header-args:elisp: :tangle experiment-bootstrap.el
:END:
...
#+begin_src elisp
(require 'elpm)
; Allow reuse of common packages across experiments
(let ((elpm-directory (expand-file-name "~/.local/emacs/site-lisp/")))
  (elpm-bootstrap)
  (elpm-use-packages
   '(use-package
      ...
      ivy
      which-key)))
;; Now init elpm for this experiment
(elpm-bootstrap)
#+end_src
...
#+end_src
*** Help
I need help figuring out what emacs has to offer -- enter a few nice tools. For what shortcuts I have available there is which-key
#+begin_src elisp
