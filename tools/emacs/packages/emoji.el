      ...
      (let ((glyph (cadr alist)))
        ...
        (if (display-graphic-p)
            ...
            (if (let ((char (elt glyph 0)))
                  ;;; TODO hack to get emojis working
                  '(if emoji--font
...
(defun emoji--parse-emoji-test ()
  ...
  (with-temp-buffer
    (insert-file-contents (substitute-env-vars "$p/emacs/admin/unidata/emoji-test.txt"))
                                ...
                                (if (equal (cadr entry) "Recent")
                                  ...
                                  (emoji--define-transient
                                   (cons (concat mname " > " (cadr entry))
                                         (cddr entry))))))))
                (message "Done with subs")
                x)
            ;; Insert an emoji.
            (let ((x (cl-loop for glyph in alist
